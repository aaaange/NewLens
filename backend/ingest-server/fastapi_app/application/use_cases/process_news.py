...
def process_news_data(news: News) -> News:
    ...
    try:
        ...
        print("번역 이후 제목: " + news.title)
        print("번역 이후 내용: " + news.content)
        logger.debug(
        ...
        if not news.keywords:
            print("내용이 없다면?: ", news.content)
            return None
        else:
            print("내용이 있다면?: ", news.keywords)
        logger.debug("Keyword extraction complete: %s", news.keywords)

        # 카테고리 분류: 기사 내용을 분석하여 적절한 카테고리 결정
        news.category = category_classification.classify_category(news.content)
        logger.debug("Category classification complete: %s", news.category)
        print("카테고리 분류: ", news.category)
